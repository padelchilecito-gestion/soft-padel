...
// --- SOUND ASSET (Base64 "Glass/Ping" Sound) ---
                                ...
                                <div className={`space-y-4 transition-opacity duration-300 ${config.promoActive ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                                    ...
                                    <div className="p-4 bg-orange-500/10 rounded-xl border border-orange-500/20 flex gap-3">
                                        ...
                                        <p className="text-xs text-orange-200">
                                            Al seleccionar <strong>4 turnos consecutivos (2 horas)</strong> en la misma cancha, se aplicará automáticamente el precio fijo de ${config.promoPrice}.
                                        </p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div key={court.id} className="bg-slate-800/50 p-4 rounded-xl border border-white/5 flex flex-col md:flex-row items-center gap-4 hover:border-white/20 transition-all">
                                    ...
                                    <div className="flex-1 w-full text-center md:text-left">
                                        ...
                                        <div className="flex flex-wrap gap-2 justify-center md:justify-start mt-1 text-xs">
                                            ...
                                            {court.isOffer1Active && <span className="text-purple-300 border border-purple-500/30 px-2 py-0.5 rounded bg-purple-500/10">Promo 1</span>}
                                        </div>
                                    </div>
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            // Initialize empty slots if undefined
                                                            for(let k=0; k<24; k++) {
                                ...
                                <div key={user.id} className="bg-slate-800/50 p-4 rounded-xl border border-white/5 flex flex-col gap-3 group hover:bg-slate-800 transition-colors shadow-lg">
                                    ...
                                    <div className="flex gap-2 mt-auto pt-3 border-t border-white/5">
                                         <button 
                                            ...
                                            className="flex-1 py-2 rounded-lg bg-slate-700/50 hover:bg-blue-600/20 text-xs font-bold text-slate-300 hover:text-blue-300 transition-colors flex items-center justify-center gap-1"
                                        >
                                            <Edit2 size={14}/> Editar
                                        </button>
                                         <button 
                                            onClick={() => handleDeleteUser(user.id)} 
                                            className="flex-1 py-2 rounded-lg bg-slate-700/50 hover:bg-red-500/20 text-xs font-bold text-slate-300 hover:text-red-400 transition-colors flex items-center justify-center gap-1"
                                        >
                            ...
                            <div className="bg-slate-800/50 p-3 rounded-lg border border-white/5 space-y-3">
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <input 
                                            type="checkbox" 
                                            checked={editingCourt.isOffer1Active} 
                                            onChange={e => setEditingCourt({...editingCourt, isOffer1Active: e.target.checked})}
                                    ...
                                    {editingCourt.isOffer1Active && (
                                        <div className="grid grid-cols-2 gap-2">
                                            <input 
                                                ...
                                                value={editingCourt.offer1Label || ''}
                                                onChange={e => setEditingCourt({...editingCourt, offer1Label: e.target.value})}
                                                className="bg-slate-700 border border-white/10 rounded px-2 py-1 text-xs text-white"
                                            />
                                            <input 
                                                type="number" 
                                                placeholder="Precio"
                                                value={editingCourt.offer1Price}
                                                onChange={e => setEditingCourt({...editingCourt, offer1Price: parseFloat(e.target.value)})}
                                                className="bg-slate-700 border border-white/10 rounded px-2 py-1 text-xs text-white"
                                            />
                                        </div>
                                    )}
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <input 
                                            type="checkbox" 
                                            checked={editingCourt.isOffer2Active} 
                                            onChange={e => setEditingCourt({...editingCourt, isOffer2Active: e.target.checked})}
                                            className="rounded bg-slate-700 border-white/10"
                                        />
                                    </div>
                                    {editingCourt.isOffer2Active && (
                                        <div className="grid grid-cols-2 gap-2">
                                            <input 
                                                type="text" 
                                                placeholder="Etiqueta (Ej: Socio)"
                                                value={editingCourt.offer2Label || ''}
                                                onChange={e => setEditingCourt({...editingCourt, offer2Label: e.target.value})}
                                                className="bg-slate-700 border border-white/10 rounded px-2 py-1 text-xs text-white"
                                            />
                                            <input 
                                                type="number" 
                                                placeholder="Precio"
                                                value={editingCourt.offer2Price}
                                                onChange={e => setEditingCourt({...editingCourt, offer2Price: parseFloat(e.target.value)})}
                                                className="bg-slate-700 border border-white/10 rounded px-2 py-1 text-xs text-white"
                                            />
                                        </div>
                                    )}
...
const App = () => {
  ...
  const handleToggleRecurring = (id: string) => {
      setBookings(prev => prev.map(b => b.id === id ? { ...b, isRecurring: !b.isRecurring } : b));
  };
