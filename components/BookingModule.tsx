...
const formatMoney = (amount?: number | null) => (amount || 0).toLocaleString('es-AR', { style: 'currency', currency: 'ARS' });
...
export const BookingModule: React.FC<BookingModuleProps> = ({ bookings, courts, config, onUpdateStatus, onToggleRecurring, onUpdateBooking, onAddBooking }) => {
  ...
  const handleFormSave = (booking: Booking) => {
      if (editingBooking) onUpdateBooking(booking);
      else onAddBooking(booking);
      setIsFormModalOpen(false);
      setEditingBooking(null);
      if (booking.paymentMethod) openPaymentModal(booking, booking.paymentMethod);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 h-full flex flex-col max-w-3xl mx-auto min-w-0" onClick={() => setActiveDropdownId(null)}>
      {/* Barra de Control Glass */}
      <div className="glass-panel p-3 rounded-xl flex flex-col sm:flex-row justify-between items-center gap-4 sticky top-0 z-30">
        <div className="flex items-center gap-2 glass-input p-1 rounded-lg w-full sm:w-auto border-none">
      ...
      <div className="flex-1 space-y-4 relative z-0">
          {dailyBookings.length === 0 ? (
              ...
              dailyBookings.map((booking) => {
                  ...
                  return (
                      <div key={booking.id} onClick={() => setSelectedBooking(booking)} className={`glass-panel border-0 relative group rounded-2xl transition-all cursor-pointer ${isConfirmed ? 'border-l-4 border-l-green-500' : 'border-l-4 border-l-yellow-500'} ${isDropdownActive ? 'z-50 ring-2 ring-blue-500/50' : 'z-0 hover:bg-white/5'}`}>
                          <div className="flex items-stretch">
                              <div className="w-20 sm:w-24 flex flex-col items-center justify-center p-2 sm:p-4 border-r border-white/5">
                              ...
                              <div className="flex flex-col items-end justify-center p-3 sm:p-4 gap-2 border-l border-white/5 min-w-[140px]">
                                  <div className="relative w-full">
                                      ...
                                      {isDropdownActive && (
                                          <div className="absolute right-0 top-full mt-2 w-48 glass-panel rounded-xl z-[60] overflow-hidden flex flex-col p-1 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
      ...
      {selectedBooking && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="glass-panel rounded-2xl w-full max-w-sm p-6 relative">
                  <div className="mb-6 border-b border-white/10 pb-4">
                      <div className="flex justify-between items-start">
                          ...
                          <div className="flex gap-2"><button onClick={() => handleEditClick(selectedBooking)} className="p-2 bg-white/5 rounded-lg text-blue-400 hover:bg-white/10 transition-colors"><Edit2 size={18} /></button><button onClick={() => setSelectedBooking(null)} className="p-2 bg-white/5 rounded-lg text-slate-400 hover:bg-white/10 transition-colors"><X size={18}/></button></div>
                      </div>
                  ...
                  <div className="space-y-4 mb-6">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-3 bg-white/5 rounded-lg border border-white/5"><span className="text-slate-400 text-xs block mb-1">Estado</span><span className={`px-2 py-0.5 rounded text-xs font-bold inline-block ${selectedBooking.status === BookingStatus.CONFIRMED ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>{selectedBooking.status}</span></div>
                        <div className="p-3 bg-white/5 rounded-lg border border-white/5"><span className="text-slate-400 text-xs block mb-1">Precio</span><span className="font-mono font-bold text-white text-sm">{formatMoney(selectedBooking.price)}</span></div>
                      </div>
                      <div className="p-3 bg-white/5 rounded-lg border border-white/5 flex items-center justify-between"><span className="text-slate-400 text-xs">Pago con</span><span className="text-sm font-bold text-white flex items-center gap-2">{getPaymentIcon(selectedBooking.paymentMethod)} {selectedBooking.paymentMethod || 'No registrado'}</span></div>
                  </div>
                  <div className="space-y-3">
                       ...
                       <div className="grid grid-cols-2 gap-2">
                           <button onClick={() => handleNotify(selectedBooking)} className="flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white py-3 rounded-xl font-bold text-sm border border-white/5"><MessageCircle size={16}/> WhatsApp</button>
                           <button onClick={() => { onToggleRecurring(selectedBooking.id); setSelectedBooking(null); }} className="flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white py-3 rounded-xl font-bold text-sm border border-white/5"><RefreshCw size={16}/> {selectedBooking.isRecurring ? 'Quitar Fijo' : 'Hacer Fijo'}</button>
                       </div>
      ...
      {/* MODAL DE PAGO (Reutilizado del POS pero con la misma estética) */}
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
              <div className="glass-panel rounded-2xl w-full max-w-sm p-6 relative">
                  ...
                  <div className="text-center mb-6">
                      ...
                      <div className="bg-white/5 p-4 rounded-xl border border-white/5 mt-4"><p className="text-slate-400 text-sm mb-1">Total a cobrar</p><span className="text-white font-bold text-2xl block">{formatMoney(getFinalPrice())}</span></div>
                  </div>
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="glass-panel rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5"><h3 className="text-lg font-bold text-white flex items-center gap-2">{isEditMode ? <Edit2 size={20} className="text-blue-400"/> : <Plus size={20} className="text-blue-400"/>} {isEditMode ? 'Editar Turno' : 'Nuevo Turno'}</h3><button onClick={onClose} className="text-slate-400 hover:text-white bg-white/5 p-1 rounded-full"><X size={20}/></button></div>
                <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
                    <div className="grid grid-cols-2 gap-4"><div><label className="text-xs text-slate-400 block mb-1">Fecha</label><input type="date" required value={form.date} onChange={e => setForm({...form, date: e.target.value})} className="glass-input w-full rounded-lg p-3"/></div><div><label className="text-xs text-slate-400 block mb-1">Hora</label><input type="time" required value={form.time} onChange={e => setForm({...form, time: e.target.value})} className="glass-input w-full rounded-lg p-3"/></div></div>
                    <div><label className="text-xs text-slate-400 block mb-1">Cancha</label><select value={form.courtId} onChange={e => handleCourtChange(e.target.value)} className="glass-input w-full rounded-lg p-3 cursor-pointer">{courts.map((c: Court) => <option key={c.id} value={c.id} className="bg-slate-900">{c.name}</option>)}</select></div>
                    <div className="space-y-3"><h4 className="text-xs font-bold text-slate-500 uppercase">Datos del Cliente</h4><div><input type="text" required placeholder="Nombre Completo" value={form.customerName} onChange={e => setForm({...form, customerName: e.target.value})} className="glass-input w-full rounded-lg p-3"/></div><div><input type="tel" placeholder="Teléfono (Opcional)" value={form.customerPhone} onChange={e => setForm({...form, customerPhone: e.target.value})} className="glass-input w-full rounded-lg p-3"/></div></div>
                     <div className="grid grid-cols-2 gap-4 pt-2"><div><label className="text-xs text-slate-400 block mb-1">Precio Final</label><div className="relative"><span className="absolute left-3 top-2.5 text-slate-500">$</span><input type="number" required value={form.price} onChange={e => setForm({...form, price: parseFloat(e.target.value)})} className="glass-input w-full rounded-lg p-2.5 pl-6 font-mono font-bold"/></div></div><div><label className="text-xs text-slate-400 block mb-1">Forma de Pago</label><select value={form.paymentMethod || ''} onChange={e => setForm({...form, paymentMethod: e.target.value as PaymentMethod})} className="glass-input w-full rounded-lg p-2.5 cursor-pointer"><option value="" className="bg-slate-900">Seleccionar...</option><option value={PaymentMethod.CASH} className="bg-slate-900">Efectivo</option><option value={PaymentMethod.QR} className="bg-slate-900">QR Mercado Pago</option><option value={PaymentMethod.TRANSFER} className="bg-slate-900">Transferencia</option></select></div></div>
                    <div className="flex items-center gap-2 bg-white/5 p-2.5 rounded-lg border border-white/10 w-full cursor-pointer hover:bg-white/10 mt-2"><input type="checkbox" checked={form.isRecurring} onChange={e => setForm({...form, isRecurring: e.target.checked})} className="rounded bg-slate-900 border-white/20"/><span className="text-sm text-slate-300">Es Fijo Semanal</span></div>
