...
export const Layout: React.FC<LayoutProps> = ({ children, activeView, onChangeView, config, role, onLogout }) => {
  ...
  const menuItems = [
    ...
    { id: 'reports', icon: PieChart, label: 'Reportes', roles: ['ADMIN'] }, // <--- NUEVO ITEM
    { id: 'settings', icon: Settings, label: 'Configuración', roles: ['ADMIN'] },
    { id: 'public', icon: Users, label: 'Vista Pública', roles: ['ADMIN', 'OPERATOR'] },
  ];
  
  // ... (Resto del componente igual, solo asegúrate de copiar el render completo si es necesario, aunque solo cambié el array menuItems)
  return (
    <div className={`min-h-screen bg-slate-950 text-white font-sans selection:bg-yellow-400 selection:text-black`}>
      ...
      <div className="lg:hidden flex items-center justify-between p-4 bg-slate-900/80 backdrop-blur-md border-b border-white/10 sticky top-0 z-50">
        ...
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-gray-300"><Menu /></button>
      </div>

      <div className="flex h-screen overflow-hidden relative z-10">
        <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} bg-slate-900/90 backdrop-blur-xl border-r border-white/10 flex flex-col`}>
          ...
          <nav className="flex-1 overflow-y-auto py-4">
            <ul className="space-y-1 px-3">
              {menuItems.filter(item => item.roles.includes(role)).map((item) => (
                <li key={item.id}>
                  <button onClick={() => { onChangeView(item.id); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${activeView === item.id ? `bg-gradient-to-r ${theme.primary} to-slate-800 ${theme.textOnPrimary} shadow-lg border border-white/10` : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
