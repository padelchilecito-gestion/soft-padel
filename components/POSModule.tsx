...
import { Search, ShoppingCart, Trash2, Plus, Minus, CreditCard, Banknote, QrCode, AlertTriangle } from 'lucide-react';
...
export const POSModule: React.FC<POSModuleProps> = ({ products, config, onProcessSale }) => {
  ...
  // --- LÓGICA CORREGIDA PARA VALIDAR STOCK ---
  const addToCart = (product: Product) => {
    ...
    setCart(prev => {
      ...
      if (existing) {
        // 2. Validación al sumar al existente
        if (existing.quantity >= product.stock) {
            alert(`⚠️ Stock máximo alcanzado (${product.stock} un.)`);
            return prev;
        }
  ...
  const updateQuantity = (productId: string, delta: number) => {
    setCart(prev => prev.map(item => {
      if (item.id === productId) {
        // Buscamos el producto original para saber su límite real
        const originalProduct = products.find(p => p.id === productId);
        const maxStock = originalProduct ? originalProduct.stock : item.stock;

        const newQty = item.quantity + delta;

        // 3. Validación al usar los botones +/- del carrito
        if (delta > 0 && newQty > maxStock) {
             alert(`⚠️ No puedes vender más de lo que tienes (${maxStock} un.)`);
             return item; 
        }
        
        // Evitar cantidades negativas o cero (para borrar usar el tacho)
                ...
                {filteredProducts.map(product => {
                    ...
                    return (
                        <div 
                            ...
                            className={`
                                group bg-slate-800/50 p-3 rounded-xl border border-white/5 flex flex-col transition-all
                                ${hasStock 
                                    ? 'hover:bg-slate-700/50 hover:border-blue-500/50 cursor-pointer active:scale-95' 
                                    : 'opacity-50 grayscale cursor-not-allowed border-red-900/30'}
                            `}
                        >
                            <div className="relative aspect-square rounded-lg overflow-hidden mb-3 bg-slate-900">
                                ...
                                {product.stock <= 0 ? (
                                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                                        <span className="bg-red-600 text-white text-xs font-bold px-2 py-1 rounded shadow-lg flex items-center gap-1">
                                            <AlertTriangle size={10}/> AGOTADO
                                        </span>
                                    </div>
                                ) : (
                                    product.stock <= product.minStockAlert && (
                                        <span className="absolute top-1 right-1 bg-orange-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold shadow-sm">
                            ...
                            <div className="flex justify-between items-center mt-2">
                                <span className={`text-xs font-bold ${hasStock ? 'text-slate-400' : 'text-red-400'}`}>
                                    {hasStock ? `${product.stock} un.` : 'Sin Stock'}
                                </span>
